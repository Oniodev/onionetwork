...
from onionetwork_cli.settings import Settings
...
class Application(QtWidgets.QApplication):
    ...
    def __init__(self, common):
        if common.platform == "Linux" or common.platform == "BSD":
            self.setDesktopFileName("org.onionetwork.Onionetwork")
            self.setOrganizationDomain("org.onionetwork.Onionetwork")
            self.setOrganizationName("Onionetwork")
...
def main():
    """
    The main() function implements all of the logic that the GUI version of onionetwork uses.
    """
    common = Common()
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="verbose",
        help="Log Onionetwork errors to stdout, and web errors to disk",
    )
    parser.add_argument(
        "--filenames",
        metavar="filenames",
    ...
    # Is there another onionetwork-gui running?
            ...
            for proc in psutil.process_iter(["pid", "name", "username"]):
                if proc.pid == existing_pid:
                    if (
                        ...
                        or "onionetwork" not in " ".join(proc.cmdline()).lower()
                    ):
                        still_running = False

        if still_running:
            print(f"Opening tab in existing Onionetwork window (pid {existing_pid})")

            # Make an event for the existing Onionetwork window
            if filenames:
