...
from onionetwork_cli.web import Web
...
class ReceiveMode(Mode):
    ...
    def data_dir_button_clicked(self):
        """
        Browse for a new onionetwork data directory, and save to tab settings
        """
        ...
        if selected_dir:
            # If we're running inside a flatpak package, the data dir must be inside ~/onionetwork
            if self.common.gui.is_flatpak:
                if not selected_dir.startswith(os.path.expanduser("~/onionetwork")):
