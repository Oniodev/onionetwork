...
def get_tor_windows(gpg, torkey, win_url, win_filename, expected_win_sig):
    ...
    dist_path = os.path.join(root_path, "onionetwork", "resources", "tor")
    ...
    # Copy into the onionetwork resources
...
def get_tor_macos(gpg, torkey, macos_url, macos_filename, expected_macos_sig):
    ...
    tor_lib_dir =  os.path.join(root_path, "onionetwork", "resources", "tor", "lib")
...
def update_tor_bridges():
    """
    Update the built-in Tor Bridges in onionetwork's torrc templates.
    """
    torrc_template_dir = os.path.join(
        root_path, os.pardir, "cli/onionetwork_cli/resources"
    )
