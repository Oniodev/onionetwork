...
def wix_build_dir_xml(root, data):
    ...
    if "id" in data and data["id"] == "ProgramMenuFolder":
        ...
        ET.SubElement(
            ...
            Name="onionetwork",
            Description="onionetwork",
            Target="[INSTALLDIR]onionetwork.exe",
...
def msi_package(build_path, msi_path, product_update_code):
    ...
    version_filename = os.path.join(
        build_path, "lib", "onionetwork_cli", "resources", "version.txt"
    )
    ...
    ET.SubElement(
        ...
        Id="*",
        Keywords="Installer",
        Description="onionetwork $(var.ProductVersion) Installer",
        Manufacturer="Micah Lee, et al.",
    ...
    ET.SubElement(
        ...
        SourceFile=os.path.join(
            desktop_dir, "onionetwork", "resources", "onionetwork.ico"
        ),
    )
    ...
    ET.SubElement(
        ...
        "Property",
        Id="ARPHELPLINK",
        Value="https://docs.onionetwork.org",
    )
    ET.SubElement(
        product_el,
        "Property",
        Id="ARPURLINFOABOUT",
        Value="https://onionetwork.org",
    )
    ...
    with open(os.path.join(build_path, "onionetwork.wxs"), "w") as f:
    ...
    run(
        [shutil.which("candle.exe"), "onionetwork.wxs"],
        build_path,
    )
    run(
        [shutil.which("light.exe"), "-ext", "WixUIExtension", "onionetwork.wixobj"],
        build_path,
    )

    print(f"> Prepare onionetwork.msi for signing")
    run(
        [
            shutil.which("insignia.exe"),
            "-im",
            os.path.join(build_path, "onionetwork.msi"),
        ],
        error_ok=True,
    )
    sign(os.path.join(build_path, "onionetwork.msi"))
...
def codesign(path):
    ...
    sign(os.path.join(path, "onionetwork.exe"))
    sign(os.path.join(path, "onionetwork-cli.exe"))
    sign(
        os.path.join(
            path,
            "lib",
...
def package(path):
    ...
    version_filename = os.path.join(
        root, "cli", "onionetwork_cli", "resources", "version.txt"
    )
    with open(version_filename) as f:
        version = f.read().strip()
    ...
    msi_package(
        path,
        os.path.join(desktop_dir, "dist", f"onionetwork-win64-{version}.msi"),
